  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 border-gray-700 bg-gray-900/50">
      <div className="mb-4 flex items-start gap-3">
        <Key className="mt-0.5 h-5 w-5 flex-shrink-0 text-gray-600 text-gray-400" />
        <div className="flex-1">
          <h5 className="mb-1 font-medium text-gray-900 text-white">
      ...
      {!loadingTokens && tokenLoadError && (
        <p className="mb-3 text-sm text-red-600 text-red-400">{tokenLoadError}</p>
      )}
      ...
      {!loadingTokens && availableTokens.length > 0 && (
        <>
          ...
          {tokenMode === 'stored' ? (
            <div>
              <label className="mb-2 block text-sm font-medium text-gray-700 text-gray-300">
                {t('projectWizard.step2.selectToken')}
              </label>
              <select
                value={selectedGithubToken}
                onChange={(event) => onSelectedGithubTokenChange(event.target.value)}
                className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm border-gray-600 bg-gray-800"
              >
      ...
      {!loadingTokens && availableTokens.length === 0 && (
        <div className="space-y-4">
          <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 border-blue-800 bg-blue-900/20">
